...
class Network(object):
    ...
    def _anchor_target_layer(self, rpn_cls_score, name):
        with tf.variable_scope(name):
            ...
            # set_shape设置张量a的静态维度,a必须有最少一个维度上是不确定才可以设置静态维度，reshape可以直接修改shape
    ...
    def _anchor_component(self):
        with tf.variable_scope('ANCHOR_' + 'default'):
            ...
            anchors.set_shape([None, 4])#(K*A, 4)
            anchor_length.set_shape([])#（K*A）
    ...
    def _add_losses(self, sigma_rpn=3.0):
        with tf.variable_scope('loss_' + self._tag):
            ...
            # tf.equal 返回true flase tf.where 类似np.where,返回非-1的索引
            # gather，根据给定索引取出对应的元素，把这些元素抽取出来组成新的tensor
            # 相当于把label非-1的分数取出来，重新组合，label也同样处理，剩下的只有fg=1和bg=0
            rpn_select = tf.where(tf.not_equal(rpn_label, -1))
            rpn_cls_score = tf.reshape(tf.gather(rpn_cls_score, rpn_select), [-1, 2])
            rpn_label = tf.reshape(tf.gather(rpn_label, rpn_select), [-1])
            # Cross entropy error 在这里计算交叉熵loss   #sparse_softmax_cross_entropy_with_logits：softmax和cross entropy放到一个函数里计算
            # sparse_softmax_cross_entropy_with_logits中，labels范围是（0，num-class）会先被转换层one-hot，因为logits的class是2（fg，bg） 比如label为1，那么就是（0,1）否则就是（1,0）
            # 对应的logits就是（bg，fg）格式
    ...
    def create_architecture(self, sess, mode, num_classes, tag=None, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2)):
        ...
        # handle most of the regularizer here#通常，会将正则化的结果当作Loss函数的一部分，而正则计算的一般是权重w与偏置值b
