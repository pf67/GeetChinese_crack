...
class vgg16(Network):
    ...
    def build_network(self, sess, is_training=True):
        with tf.variable_scope('vgg_16', 'vgg_16'):
            ...
            # select initializer  #truncated 生成截断正态分布的随机数
    ...
    def build_proposals(self, is_training, rpn_cls_prob, rpn_bbox_pred, rpn_cls_score):
        ...
        if is_training:
            # 经过边框修正，采样，非极大值抑制后的anchor的坐标， shape （512,5）这里都是采用网络实际输出的值进行计算
            #rois:感兴趣的区域，roi_scores：区域分数
            ...
            with tf.control_dependencies([rpn_labels]):
                ...
                #根据GT再次进行采样，选出32个fg和96个bg，以及对应label，bbox-target
                #rois （128,5） labels-（128） bbox_targets（128,44） weight（128,44）
