...
class Train:
    ...
    def train(self):
        ...
        print('Loading initial model weights from {:s}'.format(cfg.FLAGS.checkpoint_dir))
        variables = tf.global_variables()
        # Initialize all variables first
        sess.run(tf.variables_initializer(variables, name='init'))
        #var_keep_dic = self.get_variables_in_checkpoint_file(cfg.FLAGS.checkpoint_dir)
        # Get the variables to restore, ignorizing the variables to fix
        #variables_to_restore = self.net.get_variables_to_restore(variables, var_keep_dic)
        ckpt = tf.train.latest_checkpoint(cfg.FLAGS.checkpoint_dir)
        ...
        if ckpt:
            ...
            last_snapshot_iter += int(ckpt.split('_')[-1].split('.')[0])
        print('Loaded.')
        # Need to fix the variables before loading, so that the RGB weights are changed to BGR
        # For VGG16 it also changes the convolutional weights fc6 and fc7 to
        # fully connected weights
        #self.net.fix_variables(sess, cfg.FLAGS.checkpoint_dir)
        print('Fixed.')
